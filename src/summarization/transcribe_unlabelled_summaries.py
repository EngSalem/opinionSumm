...
#unlabelled_reviews = pd.DataFrame(data={'review':open(args.test,'r').readlines(),
#                                        'summary':['']*len(open(args.test,'r').readlines())})

#unlabelled_reviews.to_csv('~/public/datastore/yelp_bank/tmp_summ.csv',index=False)

## Load test file
print('Load test file ...')
test_doc = load_dataset(
    "csv", data_files='/afs/cs.pitt.edu/usr0/mse30/public/datastore/yelp_bank/tmp_summ.csv')['train']
